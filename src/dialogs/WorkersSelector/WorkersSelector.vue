    ...
    <ion-page>
        <ion-header>
            <ion-toolbar>
                ...
                <ion-title>Elegir Trabajadores</ion-title>
            ...
            <ion-toolbar>
                <ion-searchbar v-model="dynamicData.search" :animated="true" placeholder="Buscar Trabajador"></ion-searchbar>
            </ion-toolbar>
            <ion-progress-bar v-if="isLoading" type="indeterminate"></ion-progress-bar>
        </ion-header>
        <ion-content>
            <ion-list>
                <ion-item button v-for="worker in workers" @click="selectWorker(worker)" :key="worker.dni">
                    <ion-label>
                        <h2><b>{{ worker.name }}</b></h2>
                        <p>{{ worker.dni }} - {{ worker.role }}</p>
                        <p>{{ worker.team }} - {{ worker.supervisor }}</p>
                    </ion-label>
                    <ion-icon v-if="(selectedWorkerDNI && selectedWorkerDNI == worker.dni) || (allowMultipleChoice ? selectedWorkers.find((item) => {return item.dni == worker.dni}) : false)" color="primary" :icon="checkmarkOutline" slot="end"></ion-icon>
                </ion-item>
            </ion-list>
        </ion-content>
    </ion-page>
...
const selectedWorkers = ref<Array<IWorker>>([]);
...
const selectWorker = (worker: IWorker) => {
    ...
    if (selectedWorkers.value.includes(worker)){
        selectedWorkers.value = selectedWorkers.value.filter((selectedWorker) => {
            return selectedWorker.dni != worker.dni;
        });
    } else {
...
const workers = computed<Array<IWorker>>(() => {
    ...
    if (props.workersFilterCallback){
        workersList = workersList.filter((worker) => {
            return props.workersFilterCallback(worker);
        });
    }
    if (dynamicData.value.search.length > 0){
        workersList = workersList.filter((worker) => {
            return worker.name.toLowerCase().includes(dynamicData.value.search.toLowerCase()) || worker.dni.toLowerCase().includes(dynamicData.value.search.toLowerCase());
        });
    }
    return workersList;
});
...
onMounted(async () => {
    startLoading(async () => {
        workersData.value = await RequestAPI.get('/workers');
        selectedWorkers.value = props.selectedWorkersDNI.map((dni) => {
            return workersData.value.find((worker) => {
                return worker.dni == dni;
            });
        });
    })
});
