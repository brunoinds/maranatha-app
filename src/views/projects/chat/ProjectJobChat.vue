        ...
        <ion-header>
            <ion-toolbar>
                <ion-buttons slot="start">
                    <ion-back-button :defaultHref="'/projects/jobs/' + projectJobId"></ion-back-button>
                </ion-buttons>
                <ion-title>Chat Job</ion-title>
                    ...
                    <ion-item lines="none" :color="(dynamicData.highlightMessage && dynamicData.highlightMessage.id == message.id) ? 'light' : undefined">
                        ...
                        <article  class="message-item" :is-me="message.isMe">
                            ...
                            <section class="bubble">
                                <article class="message-sender" v-if="!message.isMe">
...
import { IProjectJobChatMessage, ProjectColorsDescriptor } from '@/interfaces/ProjectsInterfaces';
...
const projectJobId = route.params.id as string;
...
const dynamicData = ref<{
    ...
    replyTo?: IProjectJobChatMessage|null,
    reactTo?: IProjectJobChatMessage|null,
    ...
    downloadingProgress: Array<{
        message: IProjectJobChatMessage,
    ...
    highlightMessage: IProjectJobChatMessage|null
}>({
    text: '',
    image: null,
    document: null,
...
const chatMessagesData = ref<IProjectJobChatMessage[]>([]);
    ...
    return chatMessagesData.value.map((message) => {
        return {
            ...
            replyData: (() => {
                if (message.reply_to) {
                    return {
                        message: chatMessagesData.value.find((chatMessage) => chatMessage.id === message.reply_to) as unknown as IProjectJobChatMessage,
            ...
            user: usersData.value.find((user) => user.id === message.user_id) as IUser
        }
    });
});
...
const goToMessage = (messageId: string) => {
    ...
    setTimeout(() => {
        dynamicData.value.highlightMessage = chatMessagesData.value.find((message) => message.id === messageId) as unknown as IProjectJobChatMessage;
...
const getMessages = async () => {
    ...
    const response = await RequestAPI.get('/projects/jobs/'+projectJobId+'/chat');
    if (pauseTimer){
        return;
    }
...
const sendMessage = async () => {
    ...
    const sendMessageToServer = async () => {
        try {
            ...
            const response = await RequestAPI.post('/projects/jobs/' + projectJobId + '/chat', {
                ...message,
                id: undefined
            });
...
const openDocument = async (message: IProjectJobChatMessage, base64: string) => {
    Toolbox.share(message.document?.name as unknown as string, base64 as unknown as string)
}
const reactToMessage = (message: IProjectJobChatMessage) => {
...
const retrieveAttachmentData = async (attachmentId: string) => {
    const storage = await kvsIndexedDB({
        name: "projects-jobs-chat-attachments",
        version: 1,
    });
    ...
    const response = await RequestAPI.get('/projects/jobs/chat-attachments/' + attachmentId);
    await storage.set(attachmentId, response.attachment);
    return response.attachment;
}
const copyMessage = (message: IProjectJobChatMessage) => {
